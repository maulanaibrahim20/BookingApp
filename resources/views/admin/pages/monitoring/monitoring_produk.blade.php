                            ...
                            <tbody>
                                @foreach ($produk as $data)
                                    <tr>
                                        ...
                                        <td>
                                            <form action="/admin/monitoring_produk/" method="POST">
                                                @csrf
                                                <label class="custom-switch">
                                                    <input type="checkbox" name="custom-switch-checkbox"
                                                        class="custom-switch-input"
                                                        data-product-id="{{ $data->managementContent->id }}"
                                                        {{ $data->managementContent->active == 1 ? 'checked' : '' }}>
...
@section('js')
    <script>
        $('body').on('change', '.custom-switch-input', function() {
            ...
            $.ajax({
                method: 'POST',
                url: '/admin/monitoring_makeup/changestatus',
                data: {
                    productId: productId,
                    isChecked: isChecked,
                    _token: "{{ csrf_token() }}"
